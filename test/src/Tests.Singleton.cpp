...
class Singleton : public kstd::Singleton<Singleton> {};
...
TEST(SingletonTests, expectCreated) {
    ASSERT_DEATH(kstd::expectCreated<Singleton>(), "");
    Singleton singleton;
    EXPECT_NO_FATAL_FAILURE(kstd::expectCreated<Singleton>());
}

TEST(SingletonTests, singletonGuard) {
    ASSERT_DEATH(kstd::SingletonGuard<Singleton>{}, "");
    Singleton singleton;
    EXPECT_NO_FATAL_FAILURE(kstd::SingletonGuard<Singleton>{});
}